...
Read the EFFECT_TITLE and PHYSICAL_REFERENCE of the entire plan. Compare them against the Composite Effect Intent ID trigger phrases in the COMPOSITE EFFECTS table.
- If the overall effect matches a Composite Effect Intent ID (e.g. EFFECT_TITLE contains "aging", "desgaste", "wear", "envejecido", "vintage", "years", "antique", "timeworn", "aged", "old", or PHYSICAL_REFERENCE describes yellowed paper, faded colours, photo aging, oxidation, parchment, sunlight fading, handling marks) → **STOP immediately. Do NOT process individual artist steps at all. Output the OPENCV EXECUTION PLAN containing EXACTLY and ONLY the operations defined in the matching composite recipe — no fewer, no more. Do not append any operation from the artist's individual STEPS list. The recipe IS the complete execution plan.**
...
- "worn edges", "aged border color", "soft worn edges", "discoloured edges", "bordes desgastados", "bordes con color" → `wornedge` (standalone). Default params: `{"depth":55,"roughness":0.45,"holes":3,"holeFrac":0.45,"colorR":160,"colorG":110,"colorB":60,"strength":0.78,"seed":37}`
- "aging", "aged", "worn surface", "desgaste", "desgastado", "desgaste por años", "envejecido", "old texture", "vintage texture" → use `AGING_WEAR` composite recipe (11 steps). **Steps 1–9 are COLOR/OPTICAL ops — `erode` and ALL morphological ops are FORBIDDEN. Step 10 `wornedge` IS MANDATORY: soft amber/brown gradient tint at edges. Step 11 `tornborder` IS MANDATORY: very subtle physical edge irregularity on top of the colour tint — maxDepth MUST be 12px only (barely grazes the outermost pixels, never cuts into content). Core mechanic: Step 1 floodfill colors background as light parchment (252,247,235) → Step 2 pyrmeanshift(sp:14,sr:38) creates natural regional color variation in zones influenced by text proximity, breaking the flat uniform parchment → Steps 3–5 bilateral(diameter:13,sigmaColor:70,sigmaSpace:70)×3 widens amber halos at anti-aliased edges while preserving letter legibility → Step 6 gaussian(ksize:3) micro-sharpness reduction → Step 7 median(ksize:3) grain character → Step 8 vignette(strength:0.35,innerRadius:0.52,feather:2.0) subtle radial edge tinting — large flat bright centre zone, barely perceptible warm tint at outer edges only → Step 9 scratches(count:8,opacity:0.12,color:"light",seed:17) subtle light surface scratches — aged paper handling marks → Step 10 wornedge(depth:55,roughness:0.45,holes:3,holeFrac:0.45,colorR:160,colorG:110,colorB:60,strength:0.78,seed:37) soft amber/brown gradient aged border colour tint → Step 11 tornborder(maxDepth:12,roughness:0.65,holes:2,holeFrac:0.40,bgR:255,bgG:255,bgB:255,seed:99) MANDATORY very subtle physical edge irregularity — barely grazes outermost 12px, adds organic unevenness on top of the colour tint.** ⚠️ NEVER use erode — erode is STRUCTURAL and shrinks ALL bright regions including white box interiors. ⚠️ NEVER use gaussian AS THE HALO ENGINE. gaussian(ksize:3) IS permitted exclusively as Step 6 of AGING_WEAR.
...
|---|---|---|---|
| **desgaste / wear / surface erosion** | Years of UV and moisture exposure oxidise paper. Ink diffuses outward into adjacent fibers creating soft amber halos. This is a COLOR CHANGE in the background pixels near dark ink — NOT structural expansion of the ink. The reference physical algorithm (sepia-kernel → noise → GaussianBlur → vignette → blend) uses ONLY color/optical ops; none expand/shrink pixel regions. **Core mechanic: floodfill colors background as light parchment (252,247,235) — nearly white with a barely perceptible warm tint, preserving background transparency. Then bilateral(diameter:13,sigmaColor:70,sigmaSpace:70)×3 passes acts as the halo engine: anti-aliased text edge pixels (ΔColor≈90 from parchment) have weight≈0.44 per pass → visible amber fringe builds at text boundaries. Dark ink centers (ΔColor=210) have weight≈0.01 → ink center preserved, letter legibility maintained. White box interiors (ΔColor≈0) → unchanged. Then gaussian(ksize:3) reduces micro-contrast. Then median(ksize:3) adds grain. Then vignette(strength:0.35,innerRadius:0.52) adds subtle edge tinting. Then scratches adds fine handling marks. Finally wornedge adds the soft warm amber gradient at the edges simulating aged paper border discolouration. Then tornborder(maxDepth:12) adds a very subtle physical irregularity at the outermost pixels on top of the colour tint. ⚠️ NEVER use erode.** | Background: light parchment (252,247,235). Around every dark element: soft amber gradient 5–10px wide. White box interiors intact. Text readable. Irregular torn paper border on all four edges. | Step 1: `floodfill` (thresholding, paramsJson={"seedX":5,"seedY":5,"fillR":252,"fillG":247,"fillB":235,"loDiff":40}); Step 2: `pyrmeanshift` (smoothing, paramsJson={"sp":14,"sr":38}) — regional color variation; Step 3: `bilateral` (smoothing, paramsJson={"diameter":13,"sigmaColor":70,"sigmaSpace":70}) — halo pass 1; Step 4: `bilateral` (smoothing, paramsJson={"diameter":13,"sigmaColor":70,"sigmaSpace":70}) — halo pass 2; Step 5: `bilateral` (smoothing, paramsJson={"diameter":13,"sigmaColor":70,"sigmaSpace":70}) — halo pass 3; Step 6: `gaussian` (smoothing, paramsJson={"ksize":3,"sigma":0}) — micro-sharpness reduction; Step 7: `median` (smoothing, paramsJson={"ksize":3}) — grain character; Step 8: `vignette` (smoothing, paramsJson={"strength":0.35,"innerRadius":0.52,"feather":2.0}) — subtle edge tinting; Step 9: `scratches` (smoothing, paramsJson={"count":8,"opacity":0.12,"color":"light","seed":17}) — surface scratches; Step 10: `wornedge` (smoothing, paramsJson={"depth":55,"roughness":0.45,"holes":3,"holeFrac":0.45,"colorR":160,"colorG":110,"colorB":60,"strength":0.78,"seed":37}) — soft amber gradient aged border; Step 11: `tornborder` (smoothing, paramsJson={"maxDepth":12,"roughness":0.65,"holes":2,"holeFrac":0.40,"bgR":255,"bgG":255,"bgB":255,"seed":99}) — very subtle physical edge irregularity. |
...
|---|---|---|
| `AGING_WEAR` | aging, aged, years of wear, desgaste por años, envejecimiento, vintage, envejecido, old texture, worn surface | **⚠️ EXACT ORDER MANDATORY. Steps 1–9 are COLOR/OPTICAL — `erode` and ALL morphological ops are FORBIDDEN in every position. Step 10 `wornedge` and Step 11 `tornborder` ARE MANDATORY SMOOTHING-category ops — both MUST be emitted. tornborder in AGING_WEAR MUST use maxDepth:12 (subtle physical irregularity only).** Step 1: `floodfill` (thresholding) paramsJson={"seedX":5,"seedY":5,"fillR":252,"fillG":247,"fillB":235,"loDiff":40} — seeds from top-left corner, floods all connected white pixels [230,255] with light parchment (252,247,235) — nearly white with a barely perceptible warm hint, preserving background transparency. MUST be applied to the ORIGINAL image FIRST. Step 2: `pyrmeanshift` (smoothing) paramsJson={"sp":14,"sr":38} — regional color variation pass; clusters nearby similar-toned pixels into larger natural color pockets, producing the visible distinct oxidation zones and handling-mark patches across the paper surface. Step 3: `bilateral` (smoothing) paramsJson={"diameter":13,"sigmaColor":70,"sigmaSpace":70} — halo pass 1. parchment(240)–dark-ink(30) boundary ΔColor=210 → weight=exp(−210²/(2×70²))≈0.01 → ink barely fades (letter sharpness preserved). Anti-aliased edge pixels (ΔColor≈90) → weight=exp(−90²/(2×70²))≈0.44 → amber fringe builds at transitions. White box interiors (ΔColor≈0): unchanged. Step 4: `bilateral` (smoothing) paramsJson={"diameter":13,"sigmaColor":70,"sigmaSpace":70} — halo pass 2. Step 5: `bilateral` (smoothing) paramsJson={"diameter":13,"sigmaColor":70,"sigmaSpace":70} — halo pass 3. After 3 passes: amber fringe 5–10px wide at text edges; dark ink centers largely preserved (legible text); white box interiors unchanged. Step 6: `gaussian` (smoothing) paramsJson={"ksize":3,"sigma":0} — subtle micro-sharpness reduction; simulates gradual loss of crisp photographic resolution. Step 7: `median` (smoothing) paramsJson={"ksize":3} — surface grain character; fine non-linear texture variation emulating embedded dust and micro-abrasion. Step 8: `vignette` (smoothing) paramsJson={"strength":0.35,"innerRadius":0.52,"feather":2.0} — subtle radial edge tinting; large flat bright centre zone, very faint warm tint falls off slowly past 52% of corner distance preserving near-transparent paper background. Step 9: `scratches` (smoothing) paramsJson={"count":8,"opacity":0.12,"color":"light","seed":17} — subtle light surface scratches; adds faint bright streaks at random positions and angles simulating fine paper-handling marks and micro-abrasion. Step 10: `wornedge` (smoothing) paramsJson={"depth":55,"roughness":0.45,"holes":3,"holeFrac":0.45,"colorR":160,"colorG":110,"colorB":60,"strength":0.78,"seed":37} — MANDATORY soft amber/brown gradient aged border; organic random-walk boundary with quadratic colour falloff simulates aged paper edge discolouration. Step 11: `tornborder` (smoothing) paramsJson={"maxDepth":12,"roughness":0.65,"holes":2,"holeFrac":0.40,"bgR":255,"bgG":255,"bgB":255,"seed":99} — MANDATORY very subtle physical edge irregularity on top of colour tint; maxDepth 12px barely grazes outermost pixels. **⚠️ MANDATORY: `wornedge` Step 10 and `tornborder` Step 11 MUST both be emitted. ⚠️ tornborder in AGING_WEAR MUST use maxDepth:12 ONLY — never higher. ⚠️ FORBIDDEN: `erode` in any step of AGING_WEAR. erode is STRUCTURAL and destroys white box interiors. ⚠️ FORBIDDEN: `gaussian` as halo engine. EXCEPTION: `gaussian(ksize:3,sigma:0)` IS permitted as Step 6 exclusively for micro-sharpness reduction. ⚠️ CRITICAL: paramsJson keys must be EXACTLY "diameter", "sigmaColor", "sigmaSpace", "fillR", "fillG", "fillB", "sp", "sr". No abbreviations.** |
...
| `AGING_WEAR` + `EDGE_DAMAGE` | desgaste por años y maltrato en bordes, aged and damaged, old and battered | Execute all steps of `AGING_WEAR` first (steps 1–10), then execute all steps of `EDGE_DAMAGE` (steps 1–2) on the result |
...
- **`gaussian` as halo engine in `AGING_WEAR`** — NEVER use `gaussian` to create aging halos. `gaussian` is a symmetric linear blur: it diffuses BOTH text AND background with equal weight from every edge in both directions simultaneously. Applied after floodfill it turns text into a diffused smear while the background stays mostly flat. The correct halo engine is `bilateral(diameter:13, sigmaColor:70, sigmaSpace:70)` × 3 passes (Steps 3–5) — anti-aliased text edge pixels (ΔColor≈90) receive weight≈0.44 per pass and shift visibly toward warm amber; dark ink centers (ΔColor=210) receive weight≈0.01 and barely shift (letter legibility preserved); white box interiors (ΔColor≈0) are unchanged. **Exception:** `gaussian(ksize:3, sigma:0)` IS permitted as Step 6 of `AGING_WEAR` exclusively for micro-sharpness reduction.
