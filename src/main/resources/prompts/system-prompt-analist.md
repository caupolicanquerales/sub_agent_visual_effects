...
- If the overall effect matches `KEYSTONE_PHOTO` (e.g. EFFECT_TITLE contains "keystone", "keystoning", "perspective photo", "angled photo", "photo from angle", "trapezoid distortion", "convergent lines", "foto en perspectiva", "foto en ángulo", "distorsión trapezoidal", "efecto keystone", or PHYSICAL_REFERENCE describes a camera photographing a flat surface from a non-perpendicular angle where one edge of the document is visibly wider than the opposite, forming a trapezoid) → **STOP immediately. Do NOT process individual artist steps. Output the OPENCV EXECUTION PLAN containing EXACTLY and ONLY the operation(s) of the KEYSTONE_PHOTO recipe.**
- If the overall effect matches `FLAT_PHOTO` (e.g. EFFECT_TITLE contains "flat photo", "telephoto look", "tilt-shift", "shallow depth of field", "camera photo flat", "lens grain", "chromatic aberration", "color fringing", "foto plana", "foto telefoto", "tilt-shift plano", "desenfoque de plano de foco", "aberración cromática", "grano de cámara", "OCR", "ocr", "optical character recognition", "reconocimiento óptico de caracteres", "reconocimiento de caracteres", "character recognition", "preparar para OCR", "prepare for OCR", "optimizar para OCR", "OCR scan", "OCR processing", "imagen para OCR", "image for OCR", or PHYSICAL_REFERENCE describes a photographed flat document with depth-of-field blur gradient, subtle colour fringing at corners, a slight luminance gradient from a virtual light source, and film/sensor grain noise but WITHOUT trapezoid deformation of the document shape) → **STOP immediately. Do NOT process individual artist steps. Output the OPENCV EXECUTION PLAN containing EXACTLY and ONLY the operation(s) of the FLAT_PHOTO recipe.** ⚠️ EXCEPTION: if EFFECT_TITLE or PHYSICAL_REFERENCE also contains "training data", "dataset", "entrenamiento", "jitter", "variación", or "random" → the SCAN_TILT check (which ran earlier in this list) already matched and handled it via `scanjitter`; do not re-match here. ⚠️ EXCEPTION: if EFFECT_TITLE contains "real photo", "flat real", "realistic flat", "foto realista", "foto real", "realista", or "realistic" as a qualifier modifying "photo" or "flat" (e.g. "Flat Real Photo", "Realistic Flat Photo", "foto realista plana") → **SKIP this FLAT_PHOTO check immediately and continue to the FLAT_REAL_PHOTO check below.** The "flat photo" substring match MUST NOT override an explicit "real" or "realistic" qualifier in the title.
- If the overall effect matches `FLAT_REAL_PHOTO` (e.g. EFFECT_TITLE contains "flat real photo", "realistic photo", "pinhole camera", "3D photo", "realistic flat photo", "foto realista plana", "foto realista", "foto real", "foto con perspectiva 3D", "pinhole", "cámara estenopeica", "foto 3D", "perspectiva real", "realistic camera simulation", "simulación de cámara realista", "simulación pinhole", or PHYSICAL_REFERENCE contains "foto realista", "foto real", or explicitly describes a **3D pinhole camera model** with terms such as "homography", "perspective warp", "perspective transformation", "barrel distortion Brown-Conrady", "Gaussian vignetting", "depth of field ramp", "radial distortion", "non-uniform lighting Gaussian", "light source falloff", "3D tilt", "corner epsilon offset", "perspective projection H", "Perlin noise paper grain", "ink bleed into fibres", "Fresnel specular highlights", "Schlick approximation", "contact shadow", "SDF shadow", "ambient occlusion paper", "surface topography paper", "Normal Map paper" — the decisive distinguishing feature from `FLAT_PHOTO` is the presence of a **perspective homography H** that physically displaces the document corners combined with a **Gaussian intensity falloff** for vignetting; `FLAT_PHOTO` uses only affine shear and never changes corner positions) → **STOP immediately. Do NOT process individual artist steps. Output the OPENCV EXECUTION PLAN containing EXACTLY and ONLY the operation(s) of the FLAT_REAL_PHOTO recipe.** ⚠️ DISAMBIGUATION from `FLAT_PHOTO`: if EFFECT_TITLE says "flat photo" WITHOUT qualifiers like "realistic", "3D", "pinhole", "homography", or "Gaussian vignetting" → use `FLAT_PHOTO`, not `FLAT_REAL_PHOTO`. ⚠️ DISAMBIGUATION from `KEYSTONE_PHOTO`: `FLAT_REAL_PHOTO` keeps the document largely rectangular (small ε offsets, parallelogram-like, all four corners offset); `KEYSTONE_PHOTO` produces a visible trapezoid where one edge is clearly wider than the opposite.
...
| `warpperspective` | Full custom 3×3 homography matrix | `m00`–`m22` (double, identity), `outWidth`, `outHeight`, `interpolation` (String, "linear"), `borderMode` (String, **"replicate"** — BORDER_REPLICATE prevents white empty-space tears at corners when the projection overflows the source canvas; use "constant" only when a solid fill is deliberately required) |
...
| `remap` | Apply barrel or pincushion radial distortion via **inverse mapping**: for every destination pixel the undistorted source coordinate is computed as `x_src = cx + X/denom`, `denom = 1 + k1·r² + k2·r⁴`, `r` normalised to [0,1] over the image half-diagonal. `denom ≤ 0` is guarded to 1.0 (no distortion) preventing the white-eye geometry-collapse artefact | `k1` (double, 0.01 — positive=barrel, negative=pincushion; **keep in [0.001, 0.02]** — values above 0.05 can invert the denominator, creating a white "eye" hole at the centre), `k2` (double, 0.0 — higher-order correction) |
...
- "keystone", "keystoning", "efecto keystone", "trapezoid perspective", "angled photo of document", "foto en ángulo", "foto en perspectiva", "distorsión trapezoidal", "convergent edges", "foto con perspectiva trapezoidal" → **do NOT use `geometric` category** — use `keystone` category with `keystone` instead
- "tilt-shift", "telephoto look", "shallow depth of field", "chromatic aberration", "color fringing", "flat photo effect", "lens grain", "foto plana de cámara", "efecto telefoto", "aberración cromática" → **do NOT use `geometric` category** — use `flatphoto` category with `flatphoto` instead
- "flat real photo", "realistic photo", "pinhole camera simulation", "3D photo effect", "foto realista plana", "cámara estenopeica", "simulación pinhole", "foto 3D", "homography photo", "foto con perspectiva 3D" → **do NOT use `geometric` category** — use `flatrealphoto` category with `flatrealphoto` instead
...
- **NEVER fix the seed unless the user explicitly requests reproducibility**- **`seed` for `flatrealphoto`**: always emit `"seed":-1` in every `flatrealphoto` PARAMS_JSON so each call produces a unique result. With `"seed":-1` and **no explicit** `perspTL/TR/BR/BL` or `microTiltDeg` in PARAMS_JSON, the engine randomises both the Perlin grain **and** the camera geometry (corner offsets + micro-tilt) — every call looks like a photo taken from a slightly different angle. If specific geometry is needed (e.g. the plan specifies `lightX`/`lightY` for a top-left window), include those explicit values in PARAMS_JSON and they will override randomisation while grain still varies. Only use `"seed":N` (N≥0) when the user explicitly asks for reproducible output.
---
...
### CATEGORY: keystone
Use when the visual plan describes a photograph taken from a non-perpendicular angle that deforms a flat document into a trapezoid — one edge is wider (closer) and the opposite is narrower (farther away). Unlike `scansimulation` (which targets scanner bed realism with ten post-warp pipeline passes), `keystone` targets pure photographic perspective distortion grounded in the Planar Homography model and camera intrinsic physics. The document shape IS structurally deformed.

The engine runs a **four-stage pipeline**:
1. **Projective Transformation (Homography H)** — Four corner correspondences (source rectangle → destination trapezoid) are computed from the pinhole camera model. `getPerspectiveTransform` solves the 8×9 DLT system and retrieves the 3×3 matrix H via SVD. OpenCV's `warpPerspective` applies it via backward mapping: for every destination pixel `(x',y')`, find `(x,y) = H⁻¹(x',y')`.
2. **Camera Intrinsic Model (Focal Length)** — Each destination corner is derived from `depth = f·cosθY·cosθX − Y·sinθY·cosX − X·sinθX·cosθY` where `f = focalLength × max(W,H)`. Higher `focalLength` = weaker (telephoto); lower = stronger (wide-angle). This satisfies the shrinkage formula `w_new = w_old·f/(f·cosθ+z·sinθ)` for top/bottom edges.
3. **Bilinear / Lanczos-4 Interpolation** — Sub-pixel coordinates from the inverse map are resolved by the chosen interpolation kernel, preventing jagged edges in the keystoned area.
4. **Depth of Field** — A space-variant Gaussian blur is blended using `σ(y) = dofStrength·|y−y_focus|/(rows−1)`. The far edge is progressively blurred, reinforcing the perception of depth.

| operationName | What it does | Key params (type, default) |
|---|---|---|
| `keystone` | Applies projective homography derived from camera pinhole model (tiltY, tiltX), then blends a space-variant Gaussian DoF blur on the far edge. Parallel lines converge — the document becomes a trapezoid. | `tiltY` (double, 15.0 — vertical tilt degrees; +→ top farther away), `tiltX` (double, 0.0 — horizontal tilt; +→ left farther), `focalLength` (double, 1.5 — focal length multiplier of max(W,H); higher=weaker perspective), `dofEnabled` (boolean, true), `dofStrength` (double, 12.0 — σ in pixels at the far edge), `interpolation` (String, "linear" — "linear"\|"cubic"\|"lanczos4"), `borderMode` (String, "constant"), `bgR/G/B` (int, 255 — background fill colour RGB) |

#### Keystone decision hints
- "keystone", "keystoning", "efecto keystone", "trapezoid perspective", "angled photo of document", "foto en ángulo", "foto en perspectiva", "distorsión trapezoidal" → `keystone` with defaults
- "subtle keystone", "ligero keystone", "slight angle" → `keystone` paramsJson=`{"tiltY":8.0,"focalLength":2.0,"dofStrength":6.0}`
- "moderate keystone", "typical angled photo" → `keystone` paramsJson=`{"tiltY":15.0,"focalLength":1.5,"dofStrength":12.0}`
- "strong keystone", "steep angle", "gran ángulo de perspectiva" → `keystone` paramsJson=`{"tiltY":25.0,"focalLength":1.2,"dofStrength":20.0}`
- "horizontal tilt", "left/right keystone", "inclinación horizontal" → set non-zero `tiltX` (e.g. 12.0) and leave `tiltY` at 0
- "combined tilt", "tilt in two axes" → provide both `tiltY` and `tiltX` non-zero
- "no depth of field blur", "sin desenfoque" → `keystone` with `{"dofEnabled":false}`
- "high quality", "sharp keystone", "lanczos" → `keystone` with `{"interpolation":"lanczos4"}`
- "dark background", "fondo negro" → `keystone` with `{"bgR":0,"bgG":0,"bgB":0}`
- **NEVER route keystone/perspective-trapezoid requests to `geometric` category** — always use the `keystone` category
- **NEVER combine `keystone` with `AGING_WEAR` in a single step** — apply `keystone` first, then `AGING_WEAR` as a second pass if both are required
- **DISTINGUISH from `scansimulation`**: `keystone` produces a pure perspective trapezoid with DoF blur; `scansimulation` produces a scanned-document look with border noise, bleed-through, motion blur, ink bleed, JPEG compression, and other scanner artefacts. Use `keystone` when the PHYSICAL_REFERENCE is photographic (handheld camera, angled photo); use `scansimulation` when the PHYSICAL_REFERENCE is scanner-related.

---

### CATEGORY: flatphoto
Use when the visual plan describes making a flat image **look like a photograph** WITHOUT deforming its geometry into a trapezoid. The document stays rectangular — the "camera" effect comes from five optical layers applied in sequence. Unlike `keystone` (which deforms the geometry), `flatphoto` simulates a telephoto lens where the document is so far from the camera that perspective compression is negligible, so parallel lines remain parallel.

The engine runs a **five-stage pipeline**:
1. **Affine Transform (Parallel Preservation)** — A 2×3 matrix (implicit bottom row [0,0,1]) applies rotation+shear without perspective division. Parallel lines remain parallel (parallelogram, never trapezoid). Simulates a telephoto lens.
2. **Cosine Falloff / Luminance Gradient** — `I'(x,y) = I(x,y)·(1−α·d(x,y))` where `d` is the normalised distance from a virtual light source at `(lightX·W, lightY·H)`. The darkest corner receives up to `lightDecay×100 %` less brightness, simulating the Inverse Square Law without shape change.
3. **Tilt-Shift (Z-Plane Blur)** — `σ(y) = tiltShiftMaxSigma·(|y−focusRow|/maxDist)^p`. Power-law alpha map blends the sharp original with a fully-blurred reference. `p=1`=linear falloff (standard DoF); `p=2`=parabolic (very shallow DoF).
4. **Chromatic Aberration** — R and B channels are independently radially remapped: `x_src = cx + (x−cx)/(1+κ·r²)`. `κ_Red>0`=barrel (red outward); `κ_Blue<0`=pincushion (blue inward). G channel unchanged.
5. **ISO Grain** — Additive zero-mean Gaussian noise per pixel: `I''=clamp(I'+N(0,σ²))`, independent per channel.

| operationName | What it does | Key params (type, default) |
|---|---|---|
| `flatphoto` | Applies seven optical layers in sequence without changing document geometry: radial barrel distortion, affine parallelogram shear, cosine luminance gradient from a virtual light source, specular highlight spot, tilt-shift space-variant Gaussian blur, per-channel radial chromatic aberration remap, and additive ISO grain. | `affineEnabled` (boolean, true), `affineAngle` (double, 0.0 — rotation CCW degrees), `affineShearX` (double, 0.02 — keep ≤0.04; larger values split 1-px borders into double lines), `affineShearY` (double, 0.0), `borderMode` (String, "replicate"); `lightEnabled` (boolean, true), `lightX` (double, 0.5 — vignette centre X; 0.5=symmetric), `lightY` (double, 0.5 — vignette centre Y; 0.5=symmetric), `lightDecay` (double, 0.18 — corner falloff; 0=flat, 0.5=heavy vignette); `tiltShiftEnabled` (boolean, true), `focusY` (double, 0.5 — focus line as fraction of height), `tiltShiftMaxSigma` (double, 8.0 — σ cap at far edge in pixels), `tiltShiftP` (double, 1.0 — power; 1=linear, 2=parabolic); `chromaEnabled` (boolean, true), `chromaRed` (double, 0.005 — κ for R channel; realistic range 0.002–0.015), `chromaBlue` (double, -0.003 — κ for B channel; realistic range -0.002 to -0.010); `grainEnabled` (boolean, true), `grainSigma` (double, 4.0 — grain noise σ in pixel intensity units); `barrelEnabled` (boolean, true), `barrelK1` (double, 0.008 — κ1: +barrel/−pincushion; realistic range 0.003–0.030), `barrelK2` (double, 0.0 — higher-order radial correction; rarely needed); `specEnabled` (boolean, true), `specX` (double, 0.15 — specular spot centre X as fraction of width), `specY` (double, 0.12 — specular spot centre Y as fraction of height), `specStrength` (double, 0.06 — peak brightness boost 0–1; 0.06=subtle, 0.15=visible sheen), `specSigma` (double, 0.12 — spot radius as fraction of min(W,H); larger=wider softer glow) |

#### Flatphoto decision hints
- "flat photo effect", "telephoto look", "camera photo without distortion", "tilt-shift", "foto plana de cámara", "efecto telefoto", "sin keystone" → `flatphoto` with defaults
- "OCR", "ocr", "Optical Character Recognition", "Reconocimiento Óptico de Caracteres", "reconocimiento de caracteres", "character recognition", "preparar para OCR", "prepare for OCR", "optimizar para OCR", "optimize for OCR", "OCR scan", "OCR processing", "imagen para OCR", "image for OCR" → `flatphoto` with defaults (MEDIUM intensity). ⚠️ If the request also contains "training data", "dataset", "entrenamiento", "jitter", "variación", or "random" → defer to `scanjitter` (SCAN_TILT path — already checked first in STEP A).
- "subtle photo effect", "leve efecto fotográfico", "light grain" → `flatphoto` paramsJson=`{"lightDecay":0.10,"tiltShiftMaxSigma":4.0,"chromaRed":0.002,"chromaBlue":-0.001,"grainSigma":2.0,"barrelK1":0.004,"specStrength":0.03}`
- "moderate photo effect" → `flatphoto` paramsJson=`{}` (all defaults)
- "strong photo effect", "fuerte efecto fotográfico", "pronounced grain" → `flatphoto` paramsJson=`{"lightDecay":0.30,"tiltShiftMaxSigma":18.0,"chromaRed":0.012,"chromaBlue":-0.008,"grainSigma":8.0,"barrelK1":0.020,"specStrength":0.12}`
- "focus on top third", "enfoque en tercio superior" → `flatphoto` with `{"focusY":0.2}`
- "focus on bottom", "enfoque en la parte inferior" → `flatphoto` with `{"focusY":0.8}`
- "parabolic depth blur", "desenfoque parabólico", "very shallow DoF" → `flatphoto` with `{"tiltShiftP":2.0}`
- "no chromatic aberration", "sin aberración cromática" → `flatphoto` with `{"chromaEnabled":false}`
- "no grain", "sin grano" → `flatphoto` with `{"grainEnabled":false}`
- "no tilt-shift blur", "sin desenfoque de plano" → `flatphoto` with `{"tiltShiftEnabled":false}`
- "light from left side", "luz desde la izquierda" → `flatphoto` with `{"lightX":0.05,"lightY":0.5}`
- "light from top-right", "luz desde arriba a la derecha" → `flatphoto` with `{"lightX":0.85,"lightY":0.10}`
- "barrel distortion", "distorsión de barril", "subtle lens warp", "ligera distorsión de lente" → `flatphoto` with `{"barrelK1":0.015}` (stronger barrel) or `{"barrelK1":0.006}` (very subtle)
- "no barrel", "sin distorsión de barril", "perfect rectangle" → `flatphoto` with `{"barrelEnabled":false}`
- "pincushion", "distorsión de cojín" → `flatphoto` with `{"barrelK1":-0.012}`
- "specular highlight", "reflejo especular", "paper sheen", "brillo del papel", "light reflection on paper", "reflejo de luz en papel" → `flatphoto` with `{"specStrength":0.10,"specSigma":0.10}` (visible sheen)
- "subtle specular", "subtle sheen", "tenue reflejo" → `flatphoto` with `{"specStrength":0.04,"specSigma":0.15}`
- "no specular", "sin reflejo especular" → `flatphoto` with `{"specEnabled":false}`
- "specular in top-left corner" → `flatphoto` with `{"specX":0.10,"specY":0.08}` (default position)
- "specular in top-right corner", "reflejo en esquina superior derecha" → `flatphoto` with `{"specX":0.88,"specY":0.08}`
- "specular in centre", "reflejo en el centro" → `flatphoto` with `{"specX":0.5,"specY":0.5,"specSigma":0.25}`
- **NEVER route flatphoto/tilt-shift/telephoto-look requests to `geometric` category** — always use the `flatphoto` category
- **NEVER combine `flatphoto` with `AGING_WEAR` in a single step** — apply `flatphoto` first, then `AGING_WEAR` as a second pass if both are required
- **DISTINGUISH from `keystone`**: `flatphoto` keeps the document rectangular (parallelogram at most from affine shear); `keystone` physically deforms it into a trapezoid via homography. Use `flatphoto` when the request says "flat", "telephoto", "no distortion", or does not imply converging edges.

---

### CATEGORY: flatrealphoto
Use when the visual plan describes making a flat image **look like a realistic 3D pinhole camera photograph** — specifically one that is grounded in a **perspective homography** (corner offsets) and **Gaussian vignetting** rather than the telephoto/affine model of `flatphoto`. The document stays mostly rectangular but its four corners are independently displaced by small ε values that simulate viewing it from a slight 3D angle. Unlike `keystone` (which forms a visible trapezoid), the displacements here are small and the result reads as a naturally photographed flat document rather than an obviously angled shot.

The engine runs a **ten-stage pipeline** grounded in the pinhole camera model:
1. **Radial Barrel Distortion (Brown–Conrady)** — ⚠️ **Runs first on the raw source image, before any perspective warp.** Backward-mapping remap using normalised $[-1,1]$ coordinates centred at the image mid-point: $x_{norm} = (2x-W)/W$, $y_{norm} = (2y-H)/H$, $r^2 = x_{norm}^2 + y_{norm}^2$, $\text{denom} = 1 + k_1 r^2 + k_2 r^4$, $x_{src} = (x_{norm}/\text{denom}+1)\cdot W/2$. Guard: $\text{denom}\le0 \Rightarrow \text{denom}=1$ (no distortion). **Border strategy — edge replication (no white tears):** Before remapping, a **1 % internal margin** is applied: the source image is cropped symmetrically by 1 % on each side and resized back to the original canvas. This ensures the barrel inverse-map always lands on valid document content even at the extreme corners. Out-of-range remap coordinates are **clamped** to $[0, W{-}1] \times [0, H{-}1]$ (equivalent to `BORDER_REPLICATE`) instead of using the old $-1$ sentinel + `BORDER_CONSTANT` white fill that caused white-triangle / torn-edge artefacts at the image corners. Applying barrel before perspective eliminates the coordinate-singularity fold-over that appeared when distorting an already-warped trapezoid. Default `barrelK1=0.02` gives a smartphone-grade ~3\% corner bulge. k1 > 0 = barrel; k1 < 0 = pincushion. Safe range: $k_1 \in [0.001, 0.02]$.
2. **Perspective Transformation (Homography H)** — Source corners $S = [(0,0),(W,0),(W,H),(0,H)]$ are mapped to destination corners that each shift inward by a per-corner ε fraction of $\min(W,H)$. `getPerspectiveTransform` solves the 8×9 DLT system for H. `warpPerspective` with INTER_LANCZOS4 + BORDER_REPLICATE applies the slight tilt. All four corners have independent ε values (`perspTL`, `perspTR`, `perspBR`, `perspBL`) so asymmetric 3D tilts are possible.
3. **Micro Z-axis Tilt** — A sub-degree in-plane rotation (`microTiltDeg`, default 0.5°) is applied around the image centre via `getRotationMatrix2D` + `warpAffine` with INTER_LANCZOS4 + BORDER_REPLICATE. This introduces the minimum asymmetry required to trigger the brain's 3-D object recognition: one side of the document appears fractionally "taller" than the other, exactly as in a handheld photograph. Controlled by `microTiltEnabled`, `microTiltDeg`.
4. **Directional Gradient + Fill-Light Lerp + Exposure Model (Non-Uniform Lighting)** — No sigmoid is applied. The combined lighting weight is $v_{combined} = 0.65 \cdot v_{Gauss} + 0.35 \cdot (1 - v_{Dir})$ where $v_{Gauss} = \exp(-r^2/2\sigma^2)$ is the classic centred Gaussian and $v_{Dir}$ is a linear directional ramp that equals 1.0 at the light-source pixel and 0.0 at the diagonally opposite corner (sun-like gradient across the whole page). The blended weight drives the **Fill-Light lerp** directly: $I_{mask} = \text{lightMin} + (\text{lightMax} - \text{lightMin}) \cdot v_{combined}$. This means the mask never drops below `lightMin` (shadows stay bright — Global Illumination bounce from walls, sand, water) and can exceed 1.0 when `lightMax > 1.0` (overexposes the lit side for outdoor/high-key scenes). The directional component prevents the concentrated "spotlight" look of a pure Gaussian and gives a natural sunlight gradient that falls off across the whole page. **Exposure Offset** (`exposureOffset`, default 0.0) is added in float32 space after multiplication to simulate ISO lifting. **Reinhard Soft-Knee** (`reinhardEnabled`, default false): active **only when `lightMax > 1.0`**; the soft-knee is placed at `whiteLevel` itself — $\text{out} = \text{knee} + \text{excess}/(1+\text{excess}/\text{range})$ where $\text{range} = (\text{lightMax} - 1.0) \cdot \text{whiteLevel}$ — the asymptote is `whiteLevel`, preventing hard white-circle clipping without compressing valid whites. **Ambient Floor** via `Core.max(litF, ambientFloor×255)` ensures no shadow crushes to black. `lightX > 1.0` places the hotspot off-canvas to the right (e.g. `lightX=1.40` = off-frame sunlight for beach/golden-hour scenes). Light source position governs time-of-day look: noon → top-centre off-canvas (`lightY<0`); morning → upper-left (`lightX≈0.1, lightY≈0.08`); afternoon → upper-right; golden hour → far right horizon (`lightX>1.0, lightY≈0.55`); cloudy → centre with very wide sigma. Controlled by `lightMin`, `lightMax`, `lightSigma`, `exposureOffset`, `reinhardEnabled`, `whiteLevel`, `ambientFloor`.
5. **Depth of Field (Linear Ramp + Lerp)** — Linear ramp $M(y) = y/(H-1)$ drives the interpolation: $\text{Output} = (1-M) \cdot \text{sharp} + M \cdot \text{blurred}_\sigma$. Top = sharp (near), bottom = blurred (far). `dofInvert=true` reverses this. `dofTopFraction` (default 1.0) limits how far the ramp extends: 0.10 = only the top 10% is blurred — use with `dofInvert:true` for sky/horizon DoF without blurring the whole document (sunny outdoor scenes).
6. **Surface Topography (Perlin Noise + Phong Shading)** — Multi-octave fBm height map $N(x,y)$ with quintic fade. Finite-difference gradient gives the normal: $\vec{n} = (\partial N/\partial x,\, \partial N/\partial y,\, 1)$. Phong shading against the same virtual light: $I = I_{ambient} + (1-I_{ambient})\,(\vec{n}\cdot\hat{L})$. Applied as a multiplicative grain mask so paper fibre texture reacts to the light source. Controlled by `perlinEnabled`, `perlinScale`, `perlinStrength`, `perlinOctaves`, `perlinAmbient`.
7. **Ink-Substrate Integration (Variable-Kernel Bleed)** — Per-pixel weight $w \propto |I(x,y)-255|$ drives a lerp between the original and a Gaussian-blurred version: $\text{out} = (1-w)\cdot\text{orig} + w\cdot\text{blur}_\sigma$. Dark ink pixels bleed into paper fibres; white paper is untouched. Softens digital sharpness at text edges just enough to look printed. Controlled by `inkBleedEnabled`, `inkBleedSigma`, `inkBleedStr`.
8. **Specular Micro-Highlights (Schlick / Fresnel)** — Sobel image-space normals feed the Schlick approximation: $R(\theta) = R_0 + (1-R_0)(1-\cos\theta)^5$. The highlight layer is added additively, making tilted paper edges appear to have a subtle sheen. Controlled by `fresnelEnabled`, `fresnelR0`, `fresnelStrength`.
9. **Contact Shadow — Geometry-Based Ambient Occlusion** — The document boundary is obtained from the saved perspective corners (immune to lighting-induced threshold errors). `fillPoly` burns the quadrilateral into a binary mask; the mask is inverted and fed to `distanceTransform` (L2, DIST_MASK_PRECISE) so every background pixel gets its exact Euclidean distance $d$ to the nearest document edge. Shadow multiplier: $S(d) = \text{darkness}\cdot\exp(-k\cdot d^2)$ — strongest at the edge ($d \approx 1$), fades to zero beyond the shadow spread radius. This replaces the old erosion approximation which produced a Voronoi arc artefact when a bright lighting hotspot created a hole in the threshold mask. Controlled by `contactEnabled`, `contactK`, `contactDarkness`.
10. **Paper Physical-Edge Seam** — Real paper has a measurable thickness (~0.1 mm). The top edge catches ambient light producing a 1-pixel bright highlight seam; the bottom edge falls in micro-shadow producing a 1-pixel dark seam. The engine thresholds luminance at `paperEdgeThresh` to build a paper mask, then scans per-column for the topmost and bottommost paper pixel. Top-edge pixels receive `+paperEdgeHL` brightness; bottom-edge pixels receive `−paperEdgeSH` darkness. These two pixel strips make the document read as a physical object sitting on a surface rather than being pasted onto it. Controlled by `paperEdgeEnabled`, `paperEdgeHL`, `paperEdgeSH`, `paperEdgeThresh`.

| operationName | What it does | Key params (type, default) |
|---|---|---|
| `flatrealphoto` | Applies ten physics-grounded optical layers in sequence: Brown–Conrady barrel distortion remap (first, on raw source image before any warp), perspective homography from per-corner ε offsets, sub-degree micro Z-axis tilt (3-D perception cue), directional-gradient Fill-Light lerp (vCombined=0.65·vGauss+0.35·(1−vDir) → lerp between lightMin and lightMax; Reinhard soft-knee active only when lightMax > 1.0), linear-ramp depth-of-field Lerp blur, Perlin fBm surface topography with Phong shading (paper-fibre grain), variable-kernel ink-bleed integration (softens text edges realistically), Schlick Fresnel specular micro-highlights (paper sheen at grazing angles), SDF-based contact shadow (anchors document to background), and paper physical-edge seam (1-px highlight top / 1-px shadow bottom simulating paper thickness). Document stays approximately rectangular (no visible trapezoid) but acquires the natural character of a hand-held photograph. | `perspEnabled` (boolean, true), `perspTL` (double, 0.04 — top-left ε as fraction of min(W,H)), `perspTR` (double, 0.01), `perspBR` (double, 0.00), `perspBL` (double, 0.02); `barrelEnabled` (boolean, true), `barrelK1` (double, 0.02 — Brown–Conrady k1: + barrel / −pincushion; **safe range [0.001, 0.02]** — values above 0.05 can collapse the denominator to ≤0 and produce a white artefact at centre), `barrelK2` (double, 0.00 — higher-order correction); `lightEnabled` (boolean, true), `lightX` (double, 0.35 — light-source X as fraction of width [0,1]), `lightY` (double, 0.25 — light-source Y as fraction of height [0,1]), `lightSigma` (double, 0.55 — σ as fraction of image half-diagonal; smaller = tighter hotspot), `lightMin` (double, 0.40 — fill-light floor: min mask multiplier [0,1]; shadows stay at least this bright — 0.60 for sandy/water outdoor GI bounce), `lightMax` (double, 1.0 — fill-light ceiling: max mask multiplier; > 1.0 overexposes the lit side — 1.05 afternoon, 1.12 noon, 1.20 sunny beach), `exposureOffset` (double, 0.0 — additive brightness lift in [0,255] float space; simulates ISO boost — 5–8 noon, 15 sunny beach), `reinhardEnabled` (boolean, false — Reinhard soft-knee tone map; active **only when lightMax > 1.0** — prevents hard white-circle clipping without compressing valid whites), `whiteLevel` (double, 255.0 — Reinhard knee asymptote: output never exceeds this; the soft-knee is placed at this value itself — excess per-channel is rolled off smoothly via range=(lightMax−1.0)·whiteLevel); `dofEnabled` (boolean, true), `dofBlurSigma` (double, 9.0 — σ of the fully-blurred reference in pixels), `dofInvert` (boolean, false — true = blur at top instead of bottom), `dofTopFraction` (double, 1.0 — fraction of image height the DoF ramp covers; 1.0 = full-height ramp; 0.10 = top-10% only; use with dofInvert:true for sky/horizon blur without affecting the document body); `perlinEnabled` (boolean, true — step 6: paper-fibre surface grain), `perlinScale` (double, 0.012 — spatial frequency; higher = finer grain), `perlinStrength` (double, 0.08 — max brightness deviation [0,1]), `perlinOctaves` (int, 4 — fBm octave count), `perlinAmbient` (double, 0.92 — ambient term for Phong shading [0,1]); `inkBleedEnabled` (boolean, true — step 7: ink soaking into fibres), `inkBleedSigma` (double, 1.2 — max blur σ at maximum contrast in pixels), `inkBleedStr` (double, 0.55 — blend weight at maximum contrast [0,1]); `fresnelEnabled` (boolean, true — step 8: paper sheen at grazing angles), `fresnelR0` (double, 0.04 — base reflectivity of matte paper), `fresnelStrength` (double, 0.18 — scale of the entire specular pass [0,1]); `contactEnabled` (boolean, true — step 9: contact shadow anchoring document), `contactK` (double, 0.004 — shadow falloff steepness k in S(d)=darkness·exp(−k·d²); larger k = tighter rim that fades faster; 0.008–0.012 for a very tight shadow), `contactDarkness` (double, 0.55 — peak shadow multiplier at the document edge [0,1]; shadow is zero inside the document and zero far from the edge); `ambientFloor` (double, 0.20 — minimum pixel intensity after Gaussian multiply as fraction of 255 [0,1]; clamps shadows so no pixel is ever pure black — simulates real-room bounce light; 0.0 = disabled); `microTiltEnabled` (boolean, true — step 3: micro Z-axis in-plane rotation), `microTiltDeg` (double, 0.5 — rotation angle in degrees; triggers brain 3-D object recognition cue; 0.0 = no rotation); `paperEdgeEnabled` (boolean, true — step 10: paper physical-edge seam), `paperEdgeHL` (int, 30 — brightness added to top-edge pixels [0,255]; simulates ambient highlight on paper top edge), `paperEdgeSH` (int, 20 — brightness subtracted from bottom-edge pixels [0,255]; simulates micro-shadow under paper bottom edge), `paperEdgeThresh` (int, 160 — luminance threshold to distinguish paper from background); `seed` (int, -1 — RNG seed controlling **all** per-call randomisation. **-1 = random per call**: uses a two-tier jitter system. **Tier 1 (always applied, even when the param is in PARAMS_JSON):** `lightX`/`lightY` ±0.06, `barrelK1` ±0.012 (clamped [0, 0.06]), `dofBlurSigma` ±2 px (clamped [0.5, 18.0]), `lightSigma` ±0.07 (clamped [0.20, 1.0]) — ensures every call produces visually distinct lighting and barrel curvature even when the agent sends identical params. **Tier 2 (applied only when the param is absent from PARAMS_JSON):** `perspTL/TR/BR/BL` and `microTiltDeg` are randomised. ≥0 = fully reproducible: same grain AND same geometry every call; Tier 1 jitter is also disabled. Use only when the user explicitly requests reproducibility) |

#### Flatrealphoto decision hints
- "flat real photo", "realistic 3D photo", "pinhole camera simulation", "foto realista plana", "simulación de cámara pinhole", "foto con homografía", "cámara estenopeica" → `flatrealphoto` with defaults (MEDIUM intensity). **Always include `"seed":-1`** so every call produces a unique paper grain pattern.
- "subtle realistic photo", "leve efecto de cámara real" → `flatrealphoto` paramsJson=`{"perspTL":0.02,"perspTR":0.005,"perspBL":0.01,"barrelK1":0.01,"lightSigma":0.65,"lightMin":0.55,"dofBlurSigma":5.0}`
- "moderate realistic photo" → `flatrealphoto` paramsJson=`{}` (all defaults)
- "strong realistic photo", "fuerte efecto de cámara real", "pronounced 3D tilt" → `flatrealphoto` paramsJson=`{"perspTL":0.07,"perspTR":0.03,"perspBL":0.04,"perspBR":0.02,"barrelK1":0.04,"lightSigma":0.40,"lightMin":0.25,"dofBlurSigma":16.0}`
- "light from right side", "luz desde la derecha" → `flatrealphoto` with `{"lightX":0.90,"lightY":0.40}`
- "light from top-left", "top-left window light", "ventana superior izquierda", "luz desde arriba a la izquierda", "luz desde la esquina superior izquierda", "luz de ventana superior izquierda", "entra luz por la izquierda", "ventana a la izquierda" → `flatrealphoto` with `{"lightX":0.10,"lightY":0.08,"lightSigma":0.45,"lightMin":0.30}` — tight hotspot and lower ambient floor so the opposite corner (bottom-right) falls into visible shadow
- "light from top-centre", "luz desde arriba centrada" → `flatrealphoto` with `{"lightX":0.50,"lightY":0.05}`
- "light from bottom-left", "luz desde abajo a la izquierda" → `flatrealphoto` with `{"lightX":0.15,"lightY":0.90}`
- **⏱️ TIME-OF-DAY / WEATHER LIGHTING PRESETS** — use any of the following when the physical description mentions a specific lighting condition:
  - **Morning** (8–10am, indoor window light): "morning light", "morning desk", "mañana", "luz de la mañana", "morning window", "fotografía de mañana" → `{"lightX":0.10,"lightY":0.08,"lightSigma":0.45,"lightMin":0.30,"lightMax":1.0,"ambientFloor":0.20}` — tight upper-left hotspot, moderate contrast, no overexposure
  - **Noon / Midday** (overhead sun): "noon", "midday", "high noon", "mediodía", "overhead sun", "sol de mediodía", "luz cenital" → `{"lightX":0.50,"lightY":-0.10,"lightSigma":0.75,"lightMin":0.45,"lightMax":1.12,"exposureOffset":8.0,"reinhardEnabled":true,"ambientFloor":0.28}` — source off-canvas top, wide diffuse falloff, high brightness; reinhardEnabled prevents overexposure clipping
  - **Afternoon** (2–4pm, directional warm): "afternoon", "early afternoon", "tarde", "afternoon light", "post-noon", "afternoon desk" → `{"lightX":0.85,"lightY":0.20,"lightSigma":0.50,"lightMin":0.35,"lightMax":1.05,"ambientFloor":0.22}` — upper-right directional, mild overexposure on lit side
  - **Golden Hour / Late Afternoon** (low horizon sun, dramatic): "golden hour", "late afternoon", "sunset light", "hora dorada", "atardecer", "crepúsculo", "warm sunset", "low sun", "warm directional" → `{"lightX":1.20,"lightY":0.55,"lightSigma":0.38,"lightMin":0.22,"lightMax":1.10,"exposureOffset":5.0,"ambientFloor":0.18}` — off-canvas right at horizon, tight hotspot, high contrast and long shadows; lightX > 1.0 places source off-frame
  - **Cloudy / Overcast** (diffuse, no directional source): "cloudy", "overcast", "nublado", "cloudy day", "día nublado", "diffuse light", "flat outdoor light", "no direct sun", "cielo cubierto" → `{"lightX":0.50,"lightY":0.50,"lightSigma":1.50,"lightMin":0.72,"lightMax":0.92,"ambientFloor":0.55,"dofBlurSigma":4.0}` — centre source, very wide sigma (near-flat), high min+floor (GI from all directions), lightMax < 1.0 so reinhardEnabled not required
  - **Sunny Beach / Bright Outdoor** (strong off-frame sunlight, sand+water reflection): "sunny beach", "beach day", "beach sunlight", "playa soleada", "playa", "sol en la playa", "bright outdoor table", "outdoor beach", "seaside" → `{"lightX":1.40,"lightY":0.50,"lightSigma":0.70,"lightMin":0.60,"lightMax":1.20,"exposureOffset":15.0,"reinhardEnabled":true,"dofInvert":true,"dofTopFraction":0.10,"barrelK1":0.008,"ambientFloor":0.35}` — off-frame right, high GI floor from sand+water, Reinhard prevents hard white-circle clipping, sky-only DoF blur
- ⚠️ **LIGHT SOURCE SAFE-PADDING RULE**: For all standard directional presets, keep `lightX ∈ [0.05, 0.95]` and `lightY ∈ [0.05, 0.95]` — a 5 % inward padding from each edge. Off-canvas values (`lightX > 1.0` or `lightX < 0`) are only valid for the explicit off-frame presets (beach/golden-hour) and **must** be paired with `"reinhardEnabled":true` when `lightMax > 1.0` to prevent overdrive clipping artefacts. Never combine an off-canvas source with `reinhardEnabled:false` AND `lightMax > 1.05`.
- "dark corners", "vignette heavy" → `flatrealphoto` with `{"lightSigma":0.35,"lightMin":0.20}`
- "almost no vignette", "sin viñeta" → `flatrealphoto` with `{"lightEnabled":false}`
- "no barrel distortion", "sin distorsión de barril" → `flatrealphoto` with `{"barrelEnabled":false}`
- "pincushion", "cojín" → `flatrealphoto` with `{"barrelK1":-0.02}`
- "blur at top", "desenfoque arriba", "near edge blurry", "sky blur only", "desenfoque solo en el cielo", "horizon blur" → `flatrealphoto` with `{"dofInvert":true,"dofTopFraction":0.10}` — top-10% blur zone only (sky/horizon DoF without touching document body); for a full-height ramp from top to bottom use `{"dofInvert":true,"dofTopFraction":1.0}` instead
- "no depth of field", "sin profundidad de campo" → `flatrealphoto` with `{"dofEnabled":false}`
- "symmetric tilt" (all four corners offset equally) → `flatrealphoto` with equal `perspTL/TR/BR/BL` values (e.g. 0.03 each)
- "tilt left side forward" (left edge closer) → `flatrealphoto` with `{"perspTR":0.05,"perspBR":0.04,"perspTL":0.00,"perspBL":0.00}`
- "paper grain", "paper texture", "fibre texture", "grain effect", "grano de papel", "textura de papel", "fibras de papel" → enable with `{"perlinEnabled":true}` (default on); increase `perlinStrength` to 0.15–0.20 for heavy grain; reduce `perlinStrength` to 0.03–0.05 for barely perceptible
- "no paper grain", "sin grano de papel", "clean surface" → `flatrealphoto` with `{"perlinEnabled":false}`
- "fine grain", "high-frequency grain", "grano fino" → `flatrealphoto` with `{"perlinScale":0.025,"perlinOctaves":6}`
- "coarse grain", "large fibre bumps", "grano grueso" → `flatrealphoto` with `{"perlinScale":0.006,"perlinOctaves":3}`
- "heavy paper texture", "strong grain" → `flatrealphoto` with `{"perlinStrength":0.18,"perlinScale":0.015}`
- "ink bleed", "printed look", "text edge bleed", "sangrado de tinta", "tinta en fibras" → `{"inkBleedEnabled":true}` (default on); increase `inkBleedStr` to 0.75–0.90 for more visible bleed; reduce `inkBleedSigma` to 0.5–0.8 for tighter bleed
- "no ink bleed", "sharp text edges", "digital sharp", "sin sangrado" → `flatrealphoto` with `{"inkBleedEnabled":false}`
- "paper sheen", "paper gloss", "Fresnel highlight", "specular edge", "brillo del papel", "reflejo especular del papel" → `{"fresnelEnabled":true}` (default on); increase `fresnelStrength` to 0.30–0.45 for notable sheen; default 0.18 is subtle
- "no paper sheen", "matte paper only", "sin brillo", "papel mate" → `flatrealphoto` with `{"fresnelEnabled":false}`
- "strong specular", "glossy paper look", "papel brillante" → `flatrealphoto` with `{"fresnelStrength":0.40,"fresnelR0":0.08}`
- "contact shadow", "document shadow", "grounded document", "sombra de contacto", "documento anclado" → `{"contactEnabled":true}` (default on); increase `contactDarkness` to 0.75–0.90 for darker shadow; increase `contactK` to 0.008–0.012 for tighter rim
- "no contact shadow", "floating document", "sin sombra de contacto" → `flatrealphoto` with `{"contactEnabled":false}`
- "tight contact shadow", "very close shadow" → `flatrealphoto` with `{"contactK":0.010,"contactDarkness":0.70}`
- "wide spread shadow", "soft contact shadow" → `flatrealphoto` with `{"contactK":0.001,"contactDarkness":0.45}`
- "ambient light floor", "bounce light", "no black shadows", "shadows too dark", "minimum shadow brightness", "luz de rebote", "suelo de luz ambiental", "sombras demasiado oscuras" → adjust `ambientFloor` (increase toward 0.30–0.40 for visibly brighter shadows; 0.0 to disable the floor entirely)
- "micro tilt", "z-axis tilt", "subtle document rotation", "3D perception cue", "micro inclinación", "inclinación Z sutil", "slight document tilt" → adjust `microTiltDeg` (0.3–0.5 is barely perceptible but effective; 0.8–1.5 for a visible tilt; set `microTiltEnabled:false` to disable entirely)
- "paper edge highlight", "paper thickness seam", "physical edge seam", "borde físico del papel", "grosor del papel", "highlight en borde superior", "micro-sombra en borde inferior" → `paperEdgeEnabled:true` (default on); increase `paperEdgeHL` to 50–70 for a bolder highlight strip; increase `paperEdgeSH` to 35–50 for a deeper shadow strip
- "no paper edge seam", "sin borde de grosor", "sin seam" → `flatrealphoto` with `{"paperEdgeEnabled":false}`
- **NEVER route flatrealphoto requests to `geometric` or `flatphoto` categories**
- **DISTINGUISH from `flatphoto`**: `flatrealphoto` displaces document corners via homography + Gaussian vignetting (pinhole camera); `flatphoto` uses affine shear + inverse-square light + tilt-shift blur + chromatic aberration + grain (telephoto). Use `flatrealphoto` when the request explicitly mentions "realistic", "pinhole", "3D", "homography", "Gaussian vignetting", or "barrel distortion".
- **DISTINGUISH from `keystone`**: `flatrealphoto` applies small symmetric-ish ε offsets to all four corners — the result reads as a slightly imperfect flat photograph, not a visible trapezoid deformation. `keystone` applies a large asymmetric tilt that clearly shows one edge wider than the opposite. If the description says "slight perspective" or "natural photo look" → `flatrealphoto`; if it says "angled shot", "convergent edges", "trapezoid" → `keystone`.
...
| `KEYSTONE_PHOTO` | keystone, keystoning, efecto keystone, perspective photo, angled photo, photo from angle, trapezoid distortion, convergent lines, foto en perspectiva, foto en ángulo, distorsión trapezoidal, trapezoid document photo | 1. `keystone` (keystone) — uses camera pinhole model (tiltY, tiltX, focalLength) to compute 4 destination corners, solves 3×3 Homography H via DLT, applies `warpPerspective` with backward mapping (H⁻¹) and bilinear interpolation, then blends space-variant Gaussian DoF on the far edge. **Scale from INTENSITY:** LOW paramsJson=`{"tiltY":8.0,"focalLength":2.0,"dofStrength":6.0}`, MEDIUM paramsJson=`{"tiltY":15.0,"focalLength":1.5,"dofStrength":12.0}`, HIGH paramsJson=`{"tiltY":25.0,"focalLength":1.2,"dofStrength":20.0}`. If a specific numeric tilt is given, use it directly in `tiltY`. For horizontal tilt use `tiltX` instead. |
| `FLAT_PHOTO` | flat photo, telephoto look, tilt-shift, shallow depth of field, camera photo flat, lens grain, chromatic aberration, color fringing, foto plana, foto telefoto, tilt-shift plano, desenfoque de plano de foco, aberración cromática, grano de cámara, flat camera look, OCR, ocr, optical character recognition, reconocimiento óptico de caracteres, reconocimiento de caracteres, character recognition, preparar para OCR, prepare for OCR, optimizar para OCR, OCR scan, OCR processing, imagen para OCR, image for OCR | 1. `flatphoto` (flatphoto) — applies five optical layers without geometry deformation: (1) affine parallelogram shear (telephoto), (2) cosine luminance gradient `I'=I·(1−α·d)` from virtual light source, (3) space-variant Gaussian tilt-shift blur `σ(y)=tiltShiftMaxSigma·(|y−focusRow|/maxDist)^p`, (4) per-channel radial chromatic aberration remap `x_src=cx+(x−cx)/(1+κ·r²)`, (5) additive ISO grain `N(0,σ²)`. **Scale from INTENSITY:** LOW paramsJson=`{"lightDecay":0.10,"tiltShiftMaxSigma":4.0,"chromaRed":0.002,"chromaBlue":-0.001,"grainSigma":2.0}`, MEDIUM paramsJson=`{}` (all defaults), HIGH paramsJson=`{"lightDecay":0.30,"tiltShiftMaxSigma":18.0,"chromaRed":0.012,"chromaBlue":-0.008,"grainSigma":8.0}`. |
| `FLAT_REAL_PHOTO` | flat real photo, realistic photo, pinhole camera, 3D photo, realistic flat photo, foto realista plana, foto realista, foto real, foto con perspectiva 3D, pinhole, cámara estenopeica, foto 3D, perspectiva real, realistic camera simulation, simulación de cámara realista, simulación pinhole, foto con homografía, realistic pinhole simulation | 1. `flatrealphoto` (flatrealphoto) — **always include `"seed":-1` in PARAMS_JSON** so every call produces a unique paper grain (Tier 1 jitter — `lightX`/`lightY`, `barrelK1`, `dofBlurSigma`, `lightSigma` — is also applied automatically even if those params are specified). Applies ten physics-grounded layers in sequence grounded in the pinhole camera model: (1) Brown–Conrady barrel distortion remap applied **first on the raw source image** via normalised coordinates `x_norm=(2x−W)/W`, `y_norm=(2y−H)/H`, `r²=x_norm²+y_norm²`, `denom=1+k1·r²+k2·r⁴`, `x_src=(x_norm/denom+1)·W/2`; a **1 % internal safety margin** (symmetric pre-crop + scale-back to original dimensions) guarantees every inverse-map coordinate lands on document content rather than the void beyond the frame; OOB source coordinates are then **clamped to [0, W−1] × [0, H−1]** (edge replication) before the remap, and `Imgproc.remap` uses **BORDER_REPLICATE** — together these three guards (margin + clamp + replicate) eliminate the white fold-over / tear artefact that `BORDER_CONSTANT` would produce when the barrel inverse-map reaches outside the source frame; `denom≤0` guarded by reset to 1.0; (2) perspective warp via Homography H from per-corner ε offsets with **INTER_LANCZOS4 + BORDER_CONSTANT(255,255,255)** — the ε-wide border strip around the inset trapezoid represents the white table/desk surface and MUST be filled with white background, not replicated document pixels; `BORDER_CONSTANT` is safe here because barrel already ran on the clean source before this step, so the perspective warp is never followed by any sampling that could pull this white fill back into the document corners; (3) micro Z-axis in-plane rotation `microTiltDeg=0.5°` via `getRotationMatrix2D` + `warpAffine` — minimum asymmetry cue for 3-D object recognition; (4) **Directional Gradient + Fill-Light lerp**: $v_{combined}=0.65 \cdot v_{Gauss}+0.35 \cdot (1-v_{Dir})$ where $v_{Gauss}$ is the Gaussian and $v_{Dir}$ is a linear ramp (1.0 at light source, 0.0 at opposite corner); $I_{mask}=\text{lightMin}+(\text{lightMax}-\text{lightMin}) \cdot v_{combined}$; no sigmoid applied; Reinhard guard fires **only when lightMax > 1.0**, knee at `whiteLevel` itself (range=(lightMax−1.0)·whiteLevel); **ambient floor** Core.max(litF, ambientFloor×255) prevents shadow crush; lightX > 1.0 places hotspot off-canvas for extreme directional sources (beach, golden hour); (5) linear-ramp DoF lerp `Output=(1−M)·sharp + M·GaussianBlur(σ=dofBlurSigma)`; (6) Perlin fBm height map + Phong normal shading = paper-fibre grain reactive to the light source; (7) variable-kernel ink-bleed lerp `w∝|I−255|` softens text edges so they look printed rather than digital; (8) Schlick Fresnel `R(θ)=R₀+(1−R₀)(1−cosθ)⁵` adds subtle specular sheen at tilted edges; (9) geometry-based contact shadow `S(d)=darkness·exp(−k·d²)` — mask built from perspective corner polygon via `fillPoly` + `distanceTransform` (L2), shadow strongest at document edge and fading outward; immune to lighting-induced Voronoi arc artefact; (10) paper physical-edge seam: 1-px luminance-threshold top-edge highlight (`+paperEdgeHL`) and bottom-edge micro-shadow (`−paperEdgeSH`) — makes the document read as a physical object with thickness. **Scale from INTENSITY:** LOW paramsJson=`{"perspTL":0.02,"perspTR":0.005,"perspBL":0.01,"barrelK1":0.01,"lightSigma":0.65,"lightMin":0.55,"dofBlurSigma":5.0,"perlinStrength":0.04,"inkBleedStr":0.30,"fresnelStrength":0.08,"contactDarkness":0.30,"ambientFloor":0.15,"microTiltDeg":0.3}`, MEDIUM paramsJson=`{}` (all defaults), HIGH paramsJson=`{"perspTL":0.07,"perspTR":0.03,"perspBL":0.04,"perspBR":0.02,"barrelK1":0.04,"lightSigma":0.40,"lightMin":0.25,"dofBlurSigma":16.0,"perlinStrength":0.16,"inkBleedStr":0.80,"fresnelStrength":0.35,"contactDarkness":0.75,"ambientFloor":0.22,"microTiltDeg":0.7,"paperEdgeHL":45,"paperEdgeSH":30}`. ⚠️ LIGHTING NOTE: intensity levels control geometric distortion and grain strength only. For scene-specific lighting (morning, noon, afternoon, golden hour, cloudy, sunny beach), **add the corresponding lighting preset params from the Flatrealphoto decision hints section independently of INTENSITY** (e.g. sunny beach adds: `lightX:1.40, lightMin:0.60, lightMax:1.20, exposureOffset:15.0, reinhardEnabled:true, dofInvert:true, dofTopFraction:0.10`). |
...
- **`flatrealphoto` and `flatphoto` combined in a single pipeline** — Do NOT apply both in the same execution plan. They serve the same conceptual purpose (make a flat image look photographic) but with incompatible physical models. `flatphoto` is the telephoto/affine model (no corner displacement, shear-only parallelogram, chromatic aberration + ISO grain). `flatrealphoto` is the pinhole/homography model (corner ε offsets via H, Brown–Conrady barrel, Gaussian vignetting, DoF ramp). Choose exactly one based on the PHYSICAL_REFERENCE. If both are mentioned, prefer `flatrealphoto` when the description contains "homography", "pinhole", "3D", "Gaussian vignetting", or "barrel distortion k1=0.02"; prefer `flatphoto` otherwise.
- **`flatrealphoto` in `AGING_WEAR`** — NEVER add a `flatrealphoto` step to the AGING_WEAR recipe. The 12-step AGING_WEAR recipe is complete and immutable. If both a realistic photo effect and aging are requested, apply `flatrealphoto` first (as its own single-operation plan), then `AGING_WEAR` as a second pass.
